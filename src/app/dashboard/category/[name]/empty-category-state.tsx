...
export const EmptyCategoryState=({categoryName,hasEvents}:EmptyCategoryStateProps)=>{
    ...
    const {data}=useQuery({
        ...
        refetchInterval(query){
            return query.state.data?false:10000
        }
    })
        ...
        <Card contentClassName="max-w-2xl w-full flex flex-col items-center p-6" className="flex-1 flex items-center justify-center">
            ...
            <div className="mt-8 flex flex-col items-center space-x-2">
              ...
              <p className="text-sm/6 text-gray-600 mt-2">
                Need help? Check out our{" "}<a href='#' className="text-blue-600 hover:underline">documentation</a>{" "}or{" "}<a href='#'>contact support</a>
              </p>
            </div>
        </Card>
    )
