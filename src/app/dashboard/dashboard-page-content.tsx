...
export const DashboardPageContent=()=>{
  ...
  const [tripSavedData,setTripSavedData]=useState<any>()
    ...
    useEffect(()=>{
      async function fetchdata(){
        ...
        const res=await fetch('http://localhost:3000/api/category-router'
      ...
      async function travelData(){
        const tk=await tokenid()
        const res=await fetch('http://localhost:3000/api/category-routerdata'
          ,{method:"GET",headers:{'Authorization':'Bearer '+tk}});
        const data = await res.json();
        setTripSavedData(data.data)
      }
      travelData();
    },[])
                  ...
                  <div className="relative p-6 z-10">
                    ...
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center text-sm/5 text-gray-600">
                        ...
                        <span className="ml-1">{category.lastPing?category.lastPing+"ago":'Never'}</span>
                      </div>
                      <div className="flex items-center text-sm/5 text-gray-600">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <Link href={`/dashboard/category/${category.name}`} className={buttonVariants({
                        variant:'outline',size:'sm',className:'flex items-center gap-2 text-sm'
                      })}>View all <ArrowRight className="size-4"/></Link>
                      <Button variant='ghost' size='sm' className="text-gray-500 hover:text-red-600 transition-colors" onClick={()=>{setDeletingCategory(category.name)}}>
